...
public class QueueMonitor {
    ...
    private static Map<String, QueueMonitor.QueueCount> command50CountMap;
    private static Map<String, QueueMonitor.QueueCount> command100CountMap;
    private static Map<String, QueueMonitor.QueueCount> command200CountMap;
    private static Map<String, QueueMonitor.QueueCount> command500CountMap;
    private static Map<String, QueueMonitor.QueueCount> command1000CountMap;
    private static Map<String, QueueMonitor.QueueCount> command2000CountMap;
    private static Map<String, QueueMonitor.QueueCount> command10000CountMap;
    ...
    private static void addCount(Map<String, QueueMonitor.QueueCount> map, String key, int totalLTime, long maxOccurTime) {
        QueueMonitor.QueueCount curCount = (QueueMonitor.QueueCount)map.get(key);
        if (curCount == null) {
            curCount = new QueueMonitor.QueueCount();
            QueueMonitor.QueueCount old = (QueueMonitor.QueueCount)map.putIfAbsent(key, curCount);
            if (old != null) {
                curCount = old;
            }
        }
    ...
    static void map2String(StringBuilder sb, Map<String, QueueMonitor.QueueCount> map, String name) {
        sb.append("[" + name + "],,,,,\n");
        Iterator var3 = map.keySet().iterator();

        while(var3.hasNext()) {
            String key = (String)var3.next();
            sb.append(key).append(",").append(((QueueMonitor.QueueCount)map.get(key)).toString()).append("\n");
        }
